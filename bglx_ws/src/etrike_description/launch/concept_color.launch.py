...
def colored_description(desc):
    urdf_path = Path(desc, 'urdf', 'concept_color.urdf')
    robot = re.sub(r'<!--.*?-->', '', urdf_path.read_text(), flags=re.DOTALL).strip()
    if ': ' in robot:
        raise RuntimeError('robot_description contains colon-space and cannot pass through the Classic parameter parser')
    converted = subprocess.run(['gz', 'sdf', '-p', str(urdf_path)],
...
def start(context):
    ...
    return events + [server, client, state, spawn, viewer]


def generate_launch_description():
    desc = get_package_share_directory('etrike_description')
    default_world = json.loads(Path(desc, 'config', 'concept_color_world.json').read_text())['world']
