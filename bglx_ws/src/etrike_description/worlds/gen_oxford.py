...
# --- asymmetry -------------------------------------------------------------
# A perfectly square quad has 4-fold rotational symmetry, which is a
# degenerate case for scan matching: the same scan fits at 0, 90, 180 and
# 270 degrees, so the heading estimate flips between hypotheses. Real
# colleges are localisable precisely because each range is different.
# These features exist to make every viewpoint unique.

# chapel tower, northwest corner of Front Quad
box("chapel_tower", -(FRONT_QUAD + WING_DEPTH / 2), FRONT_QUAD - 3.0, 11.0,
    WING_DEPTH + 2.0, 9.0, 22.0, colour="Gazebo/Grey")

# hall buttresses along the east range only
for i in range(4):
    box("hall_buttress_%d" % i, FRONT_QUAD - 0.8, -9.0 + i * 6.0, 3.0,
        1.6, 1.2, 6.0, colour="Gazebo/Grey")

# well head, deliberately off-centre in Front Quad
cyl("quad_well", 4.5, -3.0, 0.45, 1.1, 0.9, colour="Gazebo/Grey")

# porch projecting from the west range
box("west_porch", -(FRONT_QUAD - 1.2), 6.0, 2.2, 2.4, 4.0, 4.4,
    colour="Gazebo/Grey")

# one wide gap in the Second Quad colonnade, and a statue plinth
box("garden_shed", -(SECOND_QUAD - 2.0), SECOND_QUAD_Y - 7.0, 1.2,
    3.0, 4.0, 2.4, colour="Gazebo/Wood")
box("statue_plinth", 3.0, SECOND_QUAD_Y + 4.0, 0.9, 1.2, 1.2, 1.8,
    colour="Gazebo/White")

# a single arch into the service lane, asymmetric by design
box("lane_arch_a", FRONT_QUAD + WING_DEPTH + 1.0, -6.0, WALL_HEIGHT / 2,
    2.0, WALL_THICK, WALL_HEIGHT)
