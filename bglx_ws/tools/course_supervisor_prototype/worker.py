...
FP = [(-0.20, -0.285), (1.04, -0.285), (1.04, 0.285), (-0.20, 0.285)]
...
def _geometry_audit(path, target, passage):
    ...
    for (p, q) in zip(pts, pts[1:]):
        ...
        steps = max(1, math.ceil((ds + 1.08 * abs(da)) / 0.005))
...
def velocity_text():
    ...
    for key in ('NAV', 'CM', 'LIM'):
        if key not in state:
            ...
            continue
        (v, w, stamp) = state[key]
        angle = f'{math.degrees(math.atan(0.9 * w / v)):+.1f}' if abs(v) > 1e-06 else 'N/A'
...
def reverse_grid_audit(poses, snapshots):
    ...
    for (key, md, ox, oy, heading, wx, wy, costs) in snapshots:
        ...
        for (x, y, a) in poses:
            ...
            touched = (fx >= -0.20 - margin) & (fx <= 1.04 + margin) & (abs(fy) <= 0.285 + margin)
...
def line_samples(a, b):
    (dx, dy) = (b[0] - a[0], b[1] - a[1])
    da = wrap(b[2] - a[2])
    count = max(1, math.ceil((math.hypot(dx, dy) + 1.08 * abs(da)) / 0.005))
...
def execute(path, pts, checker, label, timeout):
    ...
    for remote in ('/planner_server', '/controller_server', '/collision_monitor'):
        reply = service(GetState, remote + '/get_state', GetState.Request())
        if reply.current_state.id != 3:
            raise StageFailure('COMMAND_CHAIN', 'Command-chain readiness failed: ' + remote + ' is ' + reply.current_state.label)
...
def measure_four():
    ...
    if math.dist(robot[:2], after[:2]) > 0.01 or abs(wrap(robot[2] - after[2])) > 0.005:
        raise RuntimeError('Robot moved during Gazebo queries')
    unchanged(anchor)
    base_frame_check()
    angle = anchor[2] - robot[2]
    (c, s) = (math.cos(angle), math.sin(angle))
    pillars = [(anchor[0] + c * (x - robot[0]) - s * (y - robot[1]), anchor[1] + s * (x - robot[0]) + c * (y - robot[1]), wrap(a + angle)) for (x, y, a) in world]
    square = [(-0.3, -0.3), (0.3, -0.3), (0.3, 0.3), (-0.3, 0.3)]
    ...
    for (left, right) in (pillars[:2], pillars[2:]):
        a = left[2]
        (dx, dy) = (left[0] - right[0], left[1] - right[1])
        gap = -dx * math.sin(a) + dy * math.cos(a) - 0.6
        if abs(wrap(right[2] - a)) > 0.01 or abs(dx * math.cos(a) + dy * math.sin(a)) > 0.03 or abs(gap - 1.6) > 0.03:
            raise RuntimeError('Expected two parallel 1.6m gate pairs')
        gates.append(((left[0] + right[0]) / 2, (left[1] + right[1]) / 2, a))
...
def course_audit(points):
    ...
    for (p, q) in zip(points, points[1:]):
        if time.monotonic() - last_spin > 0.1:
            spin(0.001)
            if STOP:
                raise StageFailure('INTERRUPTED', 'Interrupted')
            last_spin = time.monotonic()
        ...
        for (j, (x, y, a)) in enumerate(gates):
            ...
            if u < 0 <= v:
                t = -u / (v - u)
                lateral = -(p[0] + t * dx - x) * math.sin(a) + (p[1] + t * dy - y) * math.cos(a)
                if abs(lateral) > 0.28:
                    raise RuntimeError('Off-centre gate crossing')
                crossings[j].append(length + t * ds)
        for sample in line_samples(p, q):
            body = xf(FP, *sample)
            minima = [min(old, poly_dist(body, b)) for (old, b) in zip(minima, boxes)]
        length += ds
    print(f'Four-box minima={minima}; length={length:.3f}m; reverse={reverse:.3f}m', flush=True)
    if exit_only:
        crossing_bad = any(crossings)
        gx, gy, ga = gates[1]
        if any((x-gx)*math.cos(ga)+(y-gy)*math.sin(ga) < 0.35
               for pose in points for x, y in xf(FP, *pose)):
            raise RuntimeError('Exit continuation re-enters gate 2')
    elif remaining_only:
        crossing_bad = len(crossings[0]) != 0 or len(crossings[1]) != 1
    else:
        crossing_bad = (any(len(v) != 1 for v in crossings)
                        or crossings[0][0] >= crossings[1][0])
    if min(minima) < 0.17 or reverse > 0.01 or crossing_bad:
        raise RuntimeError('Four-pillar geometry audit rejected')
    report.data['metrics'].update(planned_clearances=minima, length=length)
...
def local_check(p, snapshots):
    ...
    if len(local) != 1 or not okay:
        raise RuntimeError('Local next-0.5m footprint sweep blocked or outside coverage')
...
def observe_gate(j, snapshots):
    ...
    for (x, y, a) in pillars[2 * j:2 * j + 2]:
        ...
        for (key, md, ox, oy, heading, wx, wy, costs) in snapshots:
            ...
            best = max(best, int(((abs(u) < 0.4) & (abs(v) < 0.4) & (costs == 254)).sum()))
        counts.append(best)
    if min(counts) < 4:
        raise RuntimeError(f'Gate {j + 1} not sufficiently observed: {counts}; stopped before entry')
...
def check_frame():
    ...
    base_frame_check()
    if time.monotonic() - last_grid_check < 0.25:
        return
    p = pose()
...
_original_full_grid_audit = _full_grid_audit

def _full_grid_audit(poses, snapshots):
    passed = _original_full_grid_audit(poses, snapshots)
    if passed:
        return True
    for (key, md, ox, oy, heading, wx, wy, costs) in snapshots:
        touched_any = np.zeros(len(costs), dtype=bool)
        first = None
        margin = md.resolution / math.sqrt(2) + 0.011
        arc = 0.0
        previous = None
        for (x, y, a) in poses:
            if previous is not None:
                arc += math.hypot(x - previous[0], y - previous[1])
            previous = (x, y)
            (dx, dy) = (wx - x, wy - y)
            fx = dx * math.cos(a) + dy * math.sin(a)
            fy = -dx * math.sin(a) + dy * math.cos(a)
            touched = (fx >= -0.20 - margin) & (fx <= 1.04 + margin) & (abs(fy) <= 0.285 + margin) & (costs >= 254)
            touched_any |= touched
            if first is None and touched.any():
                index = int(np.flatnonzero(touched)[0])
                first = (arc, x, y, math.degrees(a), index)
        indices = np.flatnonzero(touched_any)
        if not len(indices):
            print(f'CONFLICT DETAIL {key}: no occupied cells touched; check the outside-grid result above.', flush=True)
            continue
        print(f'CONFLICT DETAIL {key}: {len(indices)} DISTINCT occupied cells; resolution={md.resolution:.3f}m', flush=True)
        if first:
            (arc, x, y, a, index) = first
            print(f'First conflict: route_arc={arc:.3f}m; body_map=({x:.3f},{y:.3f},{a:.2f}deg); cell_map=({wx[index]:.3f},{wy[index]:.3f}); cost={int(costs[index])}', flush=True)
        for index in indices[:6]:
            (px, py) = (float(wx[index]), float(wy[index]))
            nearby = []
            for (name, (bx, by, ba)) in zip(NAMES, pillars):
                (dx, dy) = (px - bx, py - by)
                u = dx * math.cos(ba) + dy * math.sin(ba)
                v = -dx * math.sin(ba) + dy * math.cos(ba)
                if abs(u) <= 0.3 + margin and abs(v) <= 0.3 + margin:
                    nearby.append(name)
            print(f"  cell=({px:.3f},{py:.3f}) cost={int(costs[index])}; near known pillar={nearby or 'none'}", flush=True)
    return False
try:
    print('FOUR-PILLAR MOTION: existing scene; 0.25m/s, no automatic reverse', flush=True)
    ...
    while True:
        try:
            ...
            if STOP or time.monotonic() > deadline:
                raise
    values = parameters('/controller_server', ['FollowPath.use_collision_detection', 'FollowPath.use_rotate_to_heading'])
    if len(values) != 2 or values[0].type != 1 or (not values[0].bool_value) or (values[1].type != 1) or values[1].bool_value:
        raise RuntimeError('Require collision detection on and rotate-to-heading off')
    measure_four()
    (gx, gy, ga) = gates[0]
    remaining_only = min(((x - gx) * math.cos(ga) + (y - gy) * math.sin(ga) for (x, y) in xf(FP, *anchor))) >= 0.35
    print('ROUTE MODE:', 'GATE 2 CONTINUATION' if remaining_only else 'FULL COURSE', flush=True)
    # PROTOTYPE_EXIT_CONTINUATION_V1
    gx2, gy2, ga2 = gates[1]
    exit_rear = min((x-gx2)*math.cos(ga2)+(y-gy2)*math.sin(ga2)
                    for x, y in xf(FP, *anchor))
    exit_only = remaining_only and exit_rear >= 0.35
    if exit_only:
        print(f'ROUTE MODE: EXIT CONTINUATION; rear beyond gate 2={exit_rear:.3f}m', flush=True)
    if remaining_only:
        body = xf(FP, *anchor)
        (gx, gy, ga) = gates[0]
        rear_clear = min(((x - gx) * math.cos(ga) + (y - gy) * math.sin(ga) for (x, y) in body))
        if rear_clear < 0.35:
            raise RuntimeError('Gate 1 not fully behind the footprint; separate transition audit required')
        (gx, gy, ga) = gates[1]
        along = (anchor[0] - gx) * math.cos(ga) + (anchor[1] - gy) * math.sin(ga)
        if along >= -0.9 and not exit_only:
            raise RuntimeError('Too close to/past gate 2 for this approach recovery')
        target = gate_point(gates[1], 2.0)
        if exit_only:
            ahead = ((target[0]-anchor[0])*math.cos(anchor[2])
                     +(target[1]-anchor[1])*math.sin(anchor[2]))
            if ahead <= 0.05:
                raise RuntimeError('Exit goal is not ahead; separate completion verification required')
        route_points = None
        for scale in (0.8, 1.0, 1.2):
            if STOP:
                raise StageFailure('INTERRUPTED', 'Interrupted')
            print(f'RESUME DIRECT: scale={scale}; current along={along:.3f}m', flush=True)
            try:
                candidate = bend(anchor, target, scale)
                course_audit(candidate)
                unchanged(anchor)
                base_frame_check()
                if not reverse_grid_audit(candidate, grid_snapshot()):
                    raise RuntimeError('Direct continuation occupancy rejected')
                route_points = candidate
                print('Selected audited exit continuation.' if exit_only else 'Selected audited direct continuation through gate 2.', flush=True)
                break
            except StageFailure:
                raise
            except RuntimeError as exc:
                if STOP:
                    raise
                print('Direct candidate rejected:', exc, flush=True)
        for setback in (2.5, 2.2, 2.0, 1.8, 1.6, 1.4, 1.2):
            if route_points is not None or exit_only:
                break
            for scale in (0.8, 1.0, 1.2):
                if STOP:
                    raise StageFailure('INTERRUPTED', 'Interrupted')
                alignment = gate_point(gates[1], -setback)
                projection = (alignment[0] - anchor[0]) * math.cos(anchor[2]) + (alignment[1] - anchor[1]) * math.sin(anchor[2])
                if projection < 0.25:
                    continue
                print(f'RESUME CANDIDATE: gate 2 setback={setback} scale={scale}', flush=True)
                try:
                    candidate = bend(anchor, alignment, scale) + line_samples(alignment, target)[1:]
                    course_audit(candidate)
                    unchanged(anchor)
                    base_frame_check()
                    if not reverse_grid_audit(candidate, grid_snapshot()):
                        raise RuntimeError('Candidate occupancy screening failed')
                    route_points = candidate
                    print('Selected fresh gate-2 continuation.', flush=True)
                    break
                except StageFailure:
                    raise
                except RuntimeError as exc:
        ...
        if route_points is None:
            raise RuntimeError('No gate-2 continuation passed; robot stays stopped')
    else:
        a1 = gate_point(gates[0], -2.5)
        c1 = gate_point(gates[0], 0.8)
        a2 = gate_point(gates[1], -2.2)
        target = gate_point(gates[1], 2.0)
        if (a1[0] - anchor[0]) * math.cos(anchor[2]) + (a1[1] - anchor[1]) * math.sin(anchor[2]) < 0.3:
            raise RuntimeError('This first-run script requires the robot before gate 1 alignment')
        segments = [bend(anchor, a1, 1.0), line_samples(a1, c1), bend(c1, a2, 1.2), line_samples(a2, target)]
        route_points = []
        for segment in segments:
            if route_points and (math.dist(route_points[-1][:2], segment[0][:2]) > 1e-06 or abs(wrap(route_points[-1][2] - segment[0][2])) > 1e-06):
    ...
    qos = QoSProfile(depth=1, reliability=ReliabilityPolicy.RELIABLE, durability=DurabilityPolicy.TRANSIENT_LOCAL)
    display = n.create_publisher(DisplayPath, '/supervised_gap/s_course_plan', qos)
    display.publish(path)
    display_timer = n.create_timer(0.5, lambda : display.publish(path))
    ...
    if not exit_only:
        observe_gate(1 if remaining_only else 0, snapshots)
    else:
        print('Exit: footprint already clear of gate 2; geometry and costmap audits retained.', flush=True)
    unchanged(anchor)
    base_frame_check()
    if os.environ.get('COURSE_EXECUTE') == '1':
        ...
        update_parameters([ParameterValue(type=1, bool_value=False), ParameterValue(type=1, bool_value=False), ParameterValue(type=3, double_value=0.6), ParameterValue(type=3, double_value=0.25)])
        execute(path, route_points, 'general_goal_checker', 'FOUR-PILLAR S-COURSE', 180)
        finish_motion()
        final = refresh_stationary_pose()
        base_frame_check()
        error = math.dist(final[:2], target[:2])
        beyond = (final[0] - gates[1][0]) * math.cos(gates[1][2]) + (final[1] - gates[1][1]) * math.sin(gates[1][2])
        if error > 0.35 or beyond < 1:
        ...
        report.data['code'] = 'AUDIT_PASS'
        print('AUDIT PASS; no motion requested.', flush=True)
except Exception as exc:
    report.fail(exc)
    ...
    try:
        ...
        print('CLEANUP INCOMPLETE:', exc, 'Stop navigation if still moving.', flush=True)
        report.data.update(code='CLEANUP_FAILED', cleanup_ok=False, detail=str(exc))
    n.destroy_node()
    rclpy.shutdown()
report.finish()
raise SystemExit(0 if report.data['code'] in ('SUCCESS', 'AUDIT_PASS') and report.data['cleanup_ok'] else 10)
