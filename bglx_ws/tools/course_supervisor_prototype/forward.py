...
try:
    print('SPEED TEST: forward target 0.25m/s; reverse remains 0.1m/s', flush=True)
    ...
    while True:
        try:
            ...
            if STOP or time.monotonic() >= deadline:
                raise
    values = parameters('/controller_server', ['FollowPath.desired_linear_vel', 'FollowPath.use_collision_detection', 'FollowPath.use_rotate_to_heading'])
    if not (values[0].type == 3 and 0 < values[0].double_value <= 0.1001 and (values[1].type == 1) and values[1].bool_value and (values[2].type == 1) and (not values[2].bool_value)):
        raise RuntimeError('Require speed <=0.1, collision detection on, rotation off')
    measure_scene()
    describe_pose('Actual start', anchor)
    candidates = []
    for setback in (4.0, 3.5, 3.0, 2.5, 2.2, 2.0, 1.9):
        target = (cx - setback * nx, cy - setback * ny, ch)
        if (target[0] - anchor[0]) * math.cos(anchor[2]) + (target[1] - anchor[1]) * math.sin(anchor[2]) < 0.3:
            continue
        for scale in (0.8, 1.0, 1.2):
            ...
            try:
                ...
                candidates.append((minimum, -length, setback, scale, path, pts))
                print(f'PASS geometry: setback={setback:.1f} scale={scale:.1f} clearance={minimum:.4f}m', flush=True)
            except RuntimeError as exc:
                print(f'Rejected setback={setback:.1f} scale={scale:.1f}: {exc}', flush=True)
    selected = None
    for candidate in rank_candidates(candidates):
        ...
        if reverse_grid_audit(candidate[5], grid_snapshot()):
            ...
            break
    if selected is None:
        raise StageFailure('NO_ROUTE', 'No forward curve passed; no motion requested')
    unchanged(anchor)
    check_frame()
    ...
    unchanged(anchor)
    check_frame()
    print(f'SELECTED: straighten {selected[2]:.1f}m before pillars; minimum={selected[0]:.4f}m', flush=True)
    execute(selected[4], selected[5], 'general_goal_checker', 'FORWARD REALIGNMENT AND CONTINUOUS PASSAGE', 240)
    finish_motion()
    refresh_stationary_pose()
    measure_scene()
    final = anchor
    ...
    if beyond < 1 or error > 0.35:
        raise RuntimeError('Final passage position verification failed')
    print(f'PASSAGE COMPLETE: goal error={error:.3f}m', flush=True)
    report.data['metrics'].update(goal_error=error, selected_setback=selected[2], planned_clearance=selected[0])
