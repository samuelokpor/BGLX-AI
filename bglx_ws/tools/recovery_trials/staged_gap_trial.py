...
try:
    print("STAGED GAP TRIAL — alignment followed by passage.", flush=True)
    ...
    if not (0 < values[0].double_value <= .1001 and values[1].bool_value
            and 0 < values[2].double_value <= .0501
            and 0 < values[3].double_value <= math.radians(1.01)):
    ...
    boxes, centres = [], []
    for name, side in zip(("pillar_test_left", "pillar_test_right"),
                          (GAP/2+.3, -GAP/2-.3)):
        unchanged(origin)
...
<material><ambient>1 0.4 0 1</ambient><diffuse>1 0.4 0 1</diffuse></material>
    ...
    while time.monotonic() < deadline:
        ...
        if stamp <= spawned or not -.05 <= age <= 4:
            continue
        md = m.metadata
        ...
        for bx, by in centres:
            dx, dy = x-bx, y-by
            counts.append(int(((abs(dx*nx+dy*ny) < .4)
                               & (abs(-dx*ny+dy*nx) < .4)).sum()))
        if min(counts) >= 4:
    ...
    alignment = (cx-2.5*nx, cy-2.5*ny, ch)
    exit_pose = (cx+4*nx, cy+4*ny, ch)
    print("Scene: gap=1.6m, angle=15deg, offset=0.5m, distance=4m")
    print("Planning alignment...", flush=True)
    path = plan(alignment, "TightSpace")
    pts = audit(path, alignment, False)
    execute(path, pts, "alignment_goal_checker", "ALIGNMENT", 150)

    actual = pose()
    position_error = math.dist(actual[:2], alignment[:2])
    heading_error = abs(wrap(actual[2]-alignment[2]))
    print(f"Achieved alignment: position error={position_error:.4f}m; "
          f"heading error={math.degrees(heading_error):.3f}deg", flush=True)
    if position_error > .05 or heading_error > math.radians(1):
        raise RuntimeError("Measured alignment outside tolerance; passage withheld")

    print("\nReplanning passage from achieved pose...", flush=True)
    path = plan(exit_pose, "GridBased")
    pts = audit(path, exit_pose, True)
    execute(path, pts, "general_goal_checker", "PASSAGE", 180)
    print(f"\nPASSAGE COMPLETED; final goal distance="
          f"{math.dist(pose()[:2], exit_pose[:2]):.3f}m", flush=True)
    print("This run validates this simulated scene, not general autonomous recovery.")
    ...
    try:
        if active is None and send_future is not None:
            try:
                ...
                print("Goal acknowledgement unresolved; stop navigation if moving.",
                      flush=True)
        ...
        if not confirmed:
            print("STOP THE NAVIGATION LAUNCH if the trike is still moving.",
                  flush=True)
    except Exception as exc:
        print("Termination unconfirmed:", exc,
              "— stop navigation if the trike is still moving.", flush=True)
    n.destroy_node()
    rclpy.shutdown()
