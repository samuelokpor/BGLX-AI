...
try:
    print("FULL CURVED ROUTE — maximum 90 seconds of motion.", flush=True)
    ...
    print("\nTRIKE WILL MOVE — full curved route", flush=True)
    send_future = follower.send_goal_async(goal)
    active = wait(send_future, 15)
    if not active.accepted:
    ...
    while not result_future.done():
        ...
        if time.monotonic()-started >= 90:
            print("TIMEOUT: route incomplete; cancelling.", flush=True)
            break
        rclpy.spin_once(n, timeout_sec=.05)
        p = pose()
    ...
    if result_future.done():
        status = result_future.result().status
        print("Route action status:", status, flush=True)
        if status == 4:
            if not wait_stopped():
                raise RuntimeError("Action succeeded but stop not confirmed")
            final = pose()
            distance = math.dist(final[:2], goal_pose[:2])
            heading = math.degrees(wrap(final[2]-goal_pose[2]))
            print(f"Final goal distance={distance:.3f}m; "
                  f"final heading error={heading:+.2f}deg", flush=True)
            if distance <= .35:
                print("FULL ROUTE POSITION TEST PASSED.", flush=True)
                print("Final heading is reported separately; "
                      "the general goal checker permits a large yaw error.",
                      flush=True)
            else:
                print("Goal position verification FAILED.", flush=True)
        else:
            print("Route did not succeed.", flush=True)
    else:
        print("Route incomplete; cancellation follows.", flush=True)
