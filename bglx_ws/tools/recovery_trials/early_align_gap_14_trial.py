...
(GAP, OFFSET, DISTANCE) = (1.4, -0.5, 6.0)
...
def audit(path, target, passage):
    ...
    if minimum < 0.17 or hits or reverse > 0.01 or (not route_ok) or (endpoint_error > 0.05):
        reasons = []
        if minimum < 0.17:
            reasons.append(f'clearance {minimum:.4f}m < {0.17:.2f}m')
        if hits:
...
try:
    print('MIRRORED GAP 1.4m — angle -15deg, offset -0.5m.', flush=True)
    ...
    for setback in (3.0, 3.5, 4.0):
        ...
        try:
            ...
            (points, minimum) = audit(candidate, exit_pose, True)
            candidates.append((minimum, label, candidate, points))
            print(f'ACCEPTABLE CANDIDATE: {label}; minimum={minimum:.4f}m', flush=True)
        except RuntimeError as exc:
            if STOP:
                raise
            print('Rejected:', exc, flush=True)
    if candidates:
        (minimum, label, selected, selected_pts) = max(candidates, key=lambda item: item[0])
        print(f'SELECTED: {label}; planned minimum={minimum:.4f}m; execution guard=0.15m', flush=True)
    if selected is None:
