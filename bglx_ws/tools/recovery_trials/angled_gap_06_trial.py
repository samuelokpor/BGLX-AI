    ...
    while not result_future.done():
        ...
        if time.monotonic()-last_print >= .5:
            ...
            print(f"t={time.monotonic()-start:6.1f} "
                  f"clr={clr:.3f} xte={xte:.3f} "
                  f"heading_error={path_heading_error(pts, p):+.2f}deg | "
                  f"{velocity_text()}",
                  flush=True)
...
def set_lookahead(values):
    ...
    request.parameters = [
        ...
        for name, value in zip(lookahead_names, values)]
    response = service(
...
<material><ambient>1 0.4 0 1</ambient><diffuse>1 0.4 0 1</diffuse></material>
    ...
    execute(selected, selected_pts, "general_goal_checker",
            "ANGLED PILLAR PASSAGE / LOOKAHEAD 0.6m", 240)
    ...
    print(f"\nPASSAGE VERIFIED: goal distance={distance:.3f}m; "
          f"final heading error={math.degrees(wrap(final[2]-ch)):+.2f}deg",
          flush=True)
    ...
    if original_settings is not None:
        if (send_future is None or
                (send_future.done() and active is not None
                 and result_future is not None and result_future.done())) and stopped():
            try:
                STOP = False
            ...
            print("Lookahead not restored: termination/stop unconfirmed. "
                  "Stop navigation and verify parameters.", flush=True)
    n.destroy_node()
    rclpy.shutdown()
