...
try:
    ...
    reverse_start = pose()
    reverse_poses = [
        (reverse_start[0]-distance*i/140*math.cos(reverse_start[2]),
         reverse_start[1]-distance*i/140*math.sin(reverse_start[2]),
         reverse_start[2])
        for i in range(141)]
    reverse_min = min(clearance(p) for p in reverse_poses)
    print(f"Reverse audit: minimum box clearance={reverse_min:.4f}m",
          flush=True)
    if reverse_min < MIN_CLEARANCE:
        raise RuntimeError("Reverse box-clearance audit failed")
    if not reverse_grid_audit(reverse_poses, snapshots):
        raise RuntimeError("Reverse costmap audit failed")

    print("Checking forward continuation before retreat...", flush=True)
    predicted_path = plan_between(reverse_poses[-1], exit_pose, "GridBased")
    audit(predicted_path, exit_pose, True)

    backup_client = ActionClient(n, BackUp, "/backup")
    if not backup_client.wait_for_server(timeout_sec=10):
        raise RuntimeError("BackUp action unavailable")
    unchanged(anchor)
    if STOP:
        raise RuntimeError("Interrupted")

    # Refresh rear occupancy immediately before sending the reverse goal.
    if not reverse_grid_audit(reverse_poses, grid_snapshot()):
        raise RuntimeError("Updated reverse costmap audit failed")
    unchanged(anchor)
    ...
    print("\nTRIKE WILL REVERSE 0.7m AT 0.1m/s", flush=True)
    send_future = backup_client.send_goal_async(goal)
    active = wait(send_future, 15)
    if not active.accepted:
        active = send_future = None
        raise RuntimeError("Reverse goal rejected")
    result_future = active.get_result_async()

    started = last_print = time.monotonic()
    minimum = clearance(reverse_start)
    while not result_future.done():
        if STOP:
            raise RuntimeError("Interrupted; cancelling reverse")
        if time.monotonic()-started > 40:
            raise RuntimeError("Reverse timeout")
        rclpy.spin_once(n, timeout_sec=.05)
        p = pose()
        dx, dy = p[0]-reverse_start[0], p[1]-reverse_start[1]
        h = reverse_start[2]
        travelled = -(dx*math.cos(h)+dy*math.sin(h))
        lateral = -dx*math.sin(h)+dy*math.cos(h)
        heading_change = wrap(p[2]-h)
        clr = clearance(p)
        minimum = min(minimum, clr)
        if clr < MIN_CLEARANCE:
            raise RuntimeError(f"Reverse clearance guard: {clr:.4f}m")
        if abs(lateral) > .03 or abs(heading_change) > math.radians(2):
            raise RuntimeError("Reverse departed from audited straight line")
        if travelled < -.03 or travelled > .75:
            raise RuntimeError("Reverse distance guard")
        if time.monotonic()-last_print >= .5:
            last_print = time.monotonic()
            print(f"t={last_print-started:5.1f} reverse={travelled:.3f}m "
                  f"lateral={lateral:+.3f}m "
                  f"heading_change={math.degrees(heading_change):+.2f}deg "
                  f"clr={clr:.4f}m | {velocity_text()}", flush=True)

    status = result_future.result().status
    stationary = wait_stopped()
    print(f"REVERSE: status={status}; stopped={stationary}; "
          f"minimum clearance={minimum:.4f}m", flush=True)
    if status != 4 or not stationary:
        raise RuntimeError("Reverse did not succeed and stop")
    active = result_future = send_future = None

    actual = pose()
    dx, dy = actual[0]-reverse_start[0], actual[1]-reverse_start[1]
    h = reverse_start[2]
    travelled = -(dx*math.cos(h)+dy*math.sin(h))
    lateral = -dx*math.sin(h)+dy*math.cos(h)
    if (not .65 <= travelled <= .75 or abs(lateral) > .03
            or abs(wrap(actual[2]-h)) > math.radians(2)):
        raise RuntimeError("Achieved retreat outside expected bounds")

    # Keep the original fixed box geometry; update only the robot start.
    anchor = actual
    print(f"Achieved reverse={travelled:.3f}m. "
          "Replanning from actual stopped pose...", flush=True)
    path = plan_between(anchor, exit_pose, "GridBased")
    pts = audit(path, exit_pose, True)
    unchanged(anchor)
    execute(path, pts, "general_goal_checker",
            "POST-RETREAT PASSAGE", 240)

    final_pose = pose()
    goal_distance = math.dist(final_pose[:2], exit_pose[:2])
    beyond = (final_pose[0]-cx)*nx+(final_pose[1]-cy)*ny
    if goal_distance > .35 or beyond < 1.:
        raise RuntimeError("Passage completion not verified")
    print(f"\nRECOVERY AND PASSAGE COMPLETED: "
          f"final goal distance={goal_distance:.3f}m", flush=True)
    exit_status = 0
