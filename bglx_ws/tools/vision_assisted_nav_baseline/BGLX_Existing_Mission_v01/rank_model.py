...
report=json.loads((a.folder/'candidates.json').read_text());ids=[c['id'] for c in report['candidates']]
result={'model':a.model,'ranking':[],'advisory_only':True};start=time.monotonic()
try:
 if not ids:raise ValueError('No candidates')
 prompt='Rank the labelled openings '+', '.join(ids)+' for further inspection by a delivery trike. Mention visible obstructions. Do not certify clearance. Reply JSON only: {"ranking":["O1"],"reason":"..."}. Use only supplied IDs; omit uncertain choices.'
 payload={'model':a.model,'stream':False,'format':'json','messages':[{'role':'user','content':prompt,'images':[base64.b64encode((a.folder/'candidates.jpg').read_bytes()).decode()]}],'options':{'temperature':0,'num_predict':200}}
