...
def course_audit(points):
    ...
    if min(minima) < 0.17 or reverse > 0.01 or (len(crossings[0]) != 0 or len(crossings[1]) != 1 if remaining_only else any((len(v) != 1 for v in crossings)) or crossings[0][0] >= crossings[1][0]):
...
def _full_grid_audit(poses, snapshots):
    ...
    for (key, md, ox, oy, heading, wx, wy, costs) in snapshots:
        ...
        for (x, y, a) in poses:
            ...
            fx = dx * math.cos(a) + dy * math.sin(a)
            fy = -dx * math.sin(a) + dy * math.cos(a)
            touched = (fx >= -0.38 - margin) & (fx <= 1.63 + margin) & (abs(fy) <= 0.52 + margin) & (costs >= 254)
            touched_any |= touched
    ...
    if remaining_only:
        ...
        for scale in (0.8, 1.0, 1.2):
            ...
            try:
                ...
                print('Selected audited direct continuation through gate 2.', flush=True)
                break
            except StageFailure:
                raise
            except RuntimeError as exc:
