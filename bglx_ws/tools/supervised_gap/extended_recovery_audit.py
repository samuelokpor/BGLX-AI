...
try:
    print('EXTENDED RECOVERY AUDIT — NO MOVEMENT / NO PARAMETER CHANGES',flush=True)
    refresh_stationary_pose()
    measure_scene()
    initial_clearance=clearance(anchor)
    if initial_clearance<.10:raise RuntimeError('Below experimental escape floor; stopped')
    planner_client=ActionClient(n,ComputePathToPose,'/compute_path_to_pose')
    if not planner_client.wait_for_server(timeout_sec=10):raise RuntimeError('Planner unavailable')
    successes=[]
    for distance in (0.,.7,1.0,1.3,1.6,2.0):
        if STOP:raise RuntimeError('Interrupted')
        unchanged(anchor);check_frame()
        print(f'\n===== INITIAL STRAIGHT RETREAT {distance:.1f}m =====',flush=True)
        snapshots=grid_snapshot()
        if distance:
            retreat_path=reverse_poses(anchor,distance)
            print(f'Retreat box minimum={min(clearance(p) for p in retreat_path):.4f}m',flush=True)
            if not escape_screen(retreat_path) or not reverse_grid_audit(retreat_path,snapshots):
                print('STRAIGHT RETREAT REJECTED',flush=True);continue
            start=retreat_path[-1]
        else:start=anchor
        for setback in (None,2.5,3.0,3.5,4.0):
            if STOP:raise RuntimeError('Interrupted')
            label='DIRECT' if setback is None else f'ALIGN {setback:.1f}m'
            print('Candidate:',label,flush=True)
            try:
                reverse=0.;cusps=[];align_min=clearance(start);align_length=0.
                if setback is None:
                    passage=plan_between(start,exit_pose,'GridBased')
                else:
                    waypoint=(cx-setback*nx,cy-setback*ny,ch)
                    alignment=plan_between(start,waypoint,'TightSpace')
                    m=mixed_metrics(alignment)
                    reverse=m['reverse'];cusps=m['cusps'];align_min=m['minimum'];align_length=m['length']
                    endpoint=m['pts'][-1]
                    print(f"Alignment: length={align_length:.3f}m reverse={reverse:.3f}m "
                          f"direction_changes={len(cusps)} min={align_min:.4f}m hits={m['hits']}",flush=True)
                    if cusps:print('Direction-change poses (arc,x,y,yaw radians):',cusps,flush=True)
                    floor=PLAN_CLEARANCE
                    if clearance(start)<PLAN_CLEARANCE:
                        if m['initial']!=-1:raise RuntimeError('Marginal start requires reverse-first escape')
                        floor=max(.10,clearance(start)-.001)
                    if m['hits'] or align_min<floor or math.dist(endpoint[:2],waypoint[:2])>.05:
                        raise RuntimeError('Alignment geometry or endpoint rejected')
                    if clearance(endpoint)<PLAN_CLEARANCE:raise RuntimeError('Alignment end clearance insufficient')
                    if any((p[0]-cx)*nx+(p[1]-cy)*ny>=0 for p in m['samples']):
                        raise RuntimeError('Alignment crosses pillar plane early')
                    # Check all of the mixed-direction alignment against both grids.
                    if not reverse_grid_audit(m['samples'],snapshots):
                        raise RuntimeError('Alignment occupancy/coverage rejected')
                    passage=plan_between(endpoint,exit_pose,'GridBased')
                    entry=passage.poses[0].pose
                    if math.dist(endpoint[:2],(entry.position.x,entry.position.y))>.03 or abs(wrap(yaw(entry.orientation)-endpoint[2]))>math.radians(1):
                        raise RuntimeError('Passage junction discontinuity')
                pts,pass_min,pass_length=audit(passage,exit_pose,True)
                total=distance+align_length+pass_length
                line=(f'PASS SCREEN: retreat={distance:.1f}m {label}; '
                      f'alignment_reverse={reverse:.3f}m cusps={len(cusps)} '
                      f'align_min={align_min:.4f}m passage_min={pass_min:.4f}m total={total:.3f}m')
                successes.append(line);print(line,flush=True)
            except RuntimeError as exc:
                if STOP:raise
                print('REJECTED:',exc,flush=True)
    print('\n===== SUMMARY =====',flush=True)
    for line in successes:print(line,flush=True)
    if not successes:print('No candidate passed these screens.',flush=True)
    print('No driving performed. Mixed-direction execution and direction-change stops are NOT validated.',flush=True)
    print('Longer retreats were audited only; existing motion runner is NOT modified to execute them.',flush=True)
    print('Costmap coverage does not prove rear visibility. Known-box geometry and map anchor are assumed static.',flush=True)
    exit_status=0
except Exception as exc:
    print('AUDIT STOPPED:',exc,flush=True);exit_status=1
finally:
