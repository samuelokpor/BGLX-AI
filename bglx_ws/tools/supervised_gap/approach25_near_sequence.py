...
code, report = run(folder / "approach25_near_scene.py", "approach25_near_setup")
if code != 0 or "APPROACH25_NEAR SCENE READY" not in report:
    raise SystemExit("Scene setup failed; no driving started.")

code, report = run(folder / "forward_realign_passage.py", "approach25_near_forward")
clean = (
    code == 0
...
if clean and not errors and "PASSAGE COMPLETE:" in report:
    print("APPROACH25_NEAR COMPLETE: forward passage succeeded.", flush=True)
    raise SystemExit(0)
...
print("APPROACH25_NEAR RECOVERY: one bounded reverse-and-forward attempt",
      flush=True)
code, report = run(
    folder / "bounded_realign_sequence.py", "approach25_near_recovery")
if code != 0 or "SEQUENCE COMPLETE:" not in report:
    raise SystemExit("Mirror recovery stopped; no further retry.")
print("APPROACH25_NEAR COMPLETE: recovery and passage succeeded.", flush=True)
