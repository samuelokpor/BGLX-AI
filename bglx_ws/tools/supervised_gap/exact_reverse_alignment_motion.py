...
def update_parameters(values):
    ...
    request.parameters = [
        ...
        for name, value in zip(changed_names, values)
    ]
    response = service(
        SetParameters, "/controller_server/set_parameters", request)
    if len(response.results) != len(values) or not all(
...
try:
    ...
    if not (
        values[0].type == 3 and 0 < values[0].double_value <= .1001
        and values[1].type == 1 and values[1].bool_value
        and values[2].type == 1 and not values[2].bool_value
        and values[3].type == 3 and 0 < values[3].double_value <= .0501
        and values[4].type == 3
        and 0 < values[4].double_value <= math.radians(1.01)
    ):
        raise RuntimeError(
            "Require speed <=0.1, collision detection on, rotation off, "
            "and alignment checker <=0.05m / 1deg.")

    measure_scene()
    initial = clearance(anchor)
    target = (cx-3.0*nx, cy-3.0*ny, ch)
    curve, max_curvature = reverse_curve(anchor, target, .8)
    floor = max(.10, initial-.001)
    minimum = min(clearance(p) for p in curve)
    length = sum(math.dist(a[:2], b[:2])
                 for a, b in zip(curve, curve[1:]))

    if initial < .10 or minimum < floor:
        raise RuntimeError("Reverse clearance screening failed")
    if clearance(curve[-1]) < initial+.05:
    ...
    print(
        f"TRIKE WILL REVERSE along {length:.3f}m curve; "
        f"audited minimum={minimum:.4f}m; "
        f"max curvature={max_curvature:.3f}/m",
        flush=True)
    ...
    while not result_future.done():
        ...
        if STOP or time.monotonic()-started > 60:
            raise RuntimeError("Interrupted or alignment timeout")
        check_frame()
        current = pose()
        clr = clearance(current)
        observed_min = min(observed_min, clr)
        nearest = min(range(len(curve)), key=lambda i:
                      math.dist(current[:2], curve[i][:2]))
        xte = min(geo["pt_seg"](current[:2], a[:2], b[:2])
                  for a, b in zip(curve, curve[1:]))
        if clr < floor:
        ...
        for key in ("NAV", "CM", "LIM"):
            entry = state.get(key)
            if entry and time.monotonic()-entry[2] < .3 and entry[0] > .01:
                raise RuntimeError("Unexpected forward command during reverse")

        if time.monotonic()-last_grid >= .5:
        ...
        if time.monotonic()-last_print >= .5:
            ...
            print(
                f"t={last_print-started:5.1f} clr={clr:.4f} "
                f"xte={xte:.4f} target={math.dist(current[:2], target[:2]):.3f}m "
                f"heading={math.degrees(wrap(current[2]-ch)):+.2f}deg",
                flush=True)
    ...
    print(
        f"ACHIEVED: setback={setback:.4f}m "
        f"lateral={lateral:+.4f}m heading={heading:+.3f}deg; "
        f"minimum observed clearance={observed_min:.4f}m",
        flush=True)

    # A geometric connector is not automatically a drivable trajectory.
    # Report the measured endpoint; never substitute the planned endpoint.
    ...
    print(
        f"Centred reference passage: minimum={reference_min:.4f}m; "
        f"occupancy clear={grid_ok}", flush=True)
    print(
        "Reverse test complete. Forward passage NOT started. "
        "The measured alignment above determines the next passage path.",
        flush=True)
except Exception as exc:
    ...
    try:
        ...
        print(
            ...
            "Stop navigation if moving; verify controller parameters.",
            flush=True)
    n.destroy_node()
    rclpy.shutdown()
