...
def spawn_scene():
    ...
    # MEASURED_PILLAR_CHECK_V2
    # Use measured world poses instead of requested spawn coordinates.
    ...
    while time.monotonic() < deadline:
        ...
        if entry is None:
            ...
            m, received = entry
            now = n.get_clock().now().nanoseconds
            age = (now-Time.from_msg(m.header.stamp).nanoseconds)/1e9
            receive_age = time.monotonic()-received
            if m.header.frame_id != "map":
                last_reason = f"Unexpected costmap frame: {m.header.frame_id}"
            elif not -.05 <= age <= 4 or receive_age > 4:
                last_reason = (
                    f"Costmap timing: stamp_age={age:.3f}s "
                    f"receive_age={receive_age:.3f}s")
            else:
                md = m.metadata
                grid = np.asarray(m.data, dtype=np.uint8).reshape(
                    md.size_y, md.size_x)
                jj, ii = np.nonzero(grid == 254)
                a = yaw(md.origin.orientation)
                u = (ii+.5)*md.resolution
                v = (jj+.5)*md.resolution
                ...
                last_reason = (
                    f"left/right lethal cells={counts}; "
                    f"stamp_age={age:.3f}s; receive_age={receive_age:.3f}s; "
                    f"resolution={md.resolution:.3f}m")
                if min(counts) >= 4:
...
# Mirrored counterpart of the previous -15 degree / -0.5m scene.
...
DISTANCE = 5.0
try:
    print("25 DEGREE NEAR SETUP: 1.4m gap, 5m ahead, +0.5m offset, +25deg",
          flush=True)
    deadline = time.monotonic()+25
    while True:
        try:
