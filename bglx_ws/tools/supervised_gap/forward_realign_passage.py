...
def forward_curve(start, target, scale):
    # The existing Hermite generator uses reverse-facing tangents.
    # Rotate its headings by pi, then convert back to forward headings.
    global ch
    original_heading = ch
    try:
        ch = wrap(ch + math.pi)
        ...
        points, curvature = reverse_curve(
            backward_start, backward_target, scale)
    finally:
        ch = original_heading
    return [(x, y, wrap(a-math.pi)) for x, y, a in points], curvature


# FORWARD_LOCAL_HORIZON_FIX
_full_grid_audit = reverse_grid_audit

def reverse_grid_audit(poses, snapshots):
    # This script executes forward paths only.
    # Check the complete route against the global map.
    ...
    # At the enforced <=0.1m/s speed, 0.5m is at least five
    # seconds of nominal travel. Live RPP collision checking
    # remains enabled as the robot and local window advance.
    horizon = 0.5
    nearby = [poses[0]]
    distance = 0.0
    ...
    print(
        f"LOCAL: next {distance:.3f}m, including the full footprint",
        flush=True)
    return _full_grid_audit(nearby, local_maps)


# EARLY_ALIGNMENT_RANKING_V1
def rank_candidates(candidates):
    if not candidates:
        return []
    best = max(c[0] for c in candidates)
    comparable = [c for c in candidates if c[0] >= best-0.005]
    remaining = [c for c in candidates if c[0] < best-0.005]
    # Larger setback means straightening farther before the pillars.
    comparable.sort(key=lambda c: (c[2], c[0], c[1]), reverse=True)
    remaining.sort(key=lambda c: (c[0], c[2], c[1]), reverse=True)
    print(
        "Ranking: clearance within 5mm of best -> earliest alignment",
        flush=True)
    return comparable + remaining


# TF_QUEUE_AND_PATH_DISPLAY_V1
...
def pose():
    # Process pending TF/clock messages before declaring data stale.
    # Keep the original freshness limit. Never wait indefinitely in motion.
    ...
    while True:
        ...
        try:
            ...
            if (not str(exc).startswith("TF not fresh:")
                    or STOP or time.monotonic() >= deadline):
                raise
...
def execute(path, pts, checker, label, timeout):
    ...
    publisher = n.create_publisher(
        DisplayPath, "/supervised_gap/selected_path", qos)
    ...
    print(
        "RViz path: /supervised_gap/selected_path "
        "(nav_msgs/Path; fixed frame map)", flush=True)
    try:
...
try:
    print("FORWARD REALIGNMENT → PASSAGE: existing scene", flush=True)
    ...
    measure_scene()
    describe_pose("Actual start", anchor)
    candidates = []

    for setback in (2.2, 2.0, 1.9):
        target = (cx-setback*nx, cy-setback*ny, ch)
        for scale in (.8, 1.0, 1.2):
            if STOP:
                raise RuntimeError("Interrupted")
            try:
                points, curvature = forward_curve(anchor, target, scale)
                points += line_samples(target, exit_pose)[1:]
                path = as_path(points)
                pts, minimum, length = audit(path, exit_pose, True)
                candidates.append(
                    (minimum, -length, setback, scale, path, pts))
                print(
                    f"PASS geometry: setback={setback:.1f} "
                    f"scale={scale:.1f} clearance={minimum:.4f}m",
                    flush=True)
            except RuntimeError as exc:
                print(
                    f"Rejected setback={setback:.1f} scale={scale:.1f}: "
                    f"{exc}", flush=True)
    ...
    for candidate in rank_candidates(candidates):
        unchanged(anchor)
        check_frame()
        if reverse_grid_audit(candidate[5], grid_snapshot()):
            selected = candidate
            break

    if selected is None:
        raise RuntimeError("No forward curve passed; no motion requested")
    ...
    check_frame()
    print(
        f"SELECTED: straighten {selected[2]:.1f}m before pillars; "
        f"minimum={selected[0]:.4f}m", flush=True)
    execute(
        selected[4], selected[5], "general_goal_checker",
        "FORWARD REALIGNMENT AND CONTINUOUS PASSAGE", 240)
    ...
    error = math.dist(final[:2], exit_pose[:2])
    if beyond < 1 or error > .35:
        raise RuntimeError("Final passage position verification failed")
    print(f"PASSAGE COMPLETE: goal error={error:.3f}m", flush=True)
