...
code, report = run(folder / "approach25_scene.py", "approach25_setup")
if code != 0 or "APPROACH25 SCENE READY" not in report:
    raise SystemExit("Scene setup failed; no driving started.")

code, report = run(folder / "forward_realign_passage.py", "approach25_forward")
clean = (
    code == 0
...
if clean and not errors and "PASSAGE COMPLETE:" in report:
    print("APPROACH25 COMPLETE: forward passage succeeded.", flush=True)
    raise SystemExit(0)
...
print("APPROACH25 RECOVERY: one bounded reverse-and-forward attempt",
      flush=True)
code, report = run(
    folder / "bounded_realign_sequence.py", "approach25_recovery")
if code != 0 or "SEQUENCE COMPLETE:" not in report:
    raise SystemExit("Mirror recovery stopped; no further retry.")
print("APPROACH25 COMPLETE: recovery and passage succeeded.", flush=True)
