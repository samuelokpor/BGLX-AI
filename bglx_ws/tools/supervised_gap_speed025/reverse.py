...
from rcl_interfaces.msg import Parameter as ParameterMsg, ParameterValue
saved = None
changed_names = ['FollowPath.allow_reversing', 'FollowPath.use_velocity_scaled_lookahead_dist', 'FollowPath.lookahead_dist']
...
try:
    print('EXACT REVERSE ALIGNMENT — MOTION TEST', flush=True)
    ...
    while True:
        try:
            ...
            if STOP or time.monotonic() > deadline:
                raise
    names = ['FollowPath.desired_linear_vel', 'FollowPath.use_collision_detection', 'FollowPath.use_rotate_to_heading', 'alignment_goal_checker.xy_goal_tolerance', 'alignment_goal_checker.yaw_goal_tolerance']
    values = parameters('/controller_server', names)
    if not (values[0].type == 3 and 0 < values[0].double_value <= 0.1001 and (values[1].type == 1) and values[1].bool_value and (values[2].type == 1) and (not values[2].bool_value) and (values[3].type == 3) and (0 < values[3].double_value <= 0.0501) and (values[4].type == 3) and (0 < values[4].double_value <= math.radians(1.01))):
        raise RuntimeError('Require speed <=0.1, collision detection on, rotation off, and alignment checker <=0.05m / 1deg.')
    measure_scene()
    initial = clearance(anchor)
    target = (cx - 3.0 * nx, cy - 3.0 * ny, ch)
    (curve, max_curvature) = reverse_curve(anchor, target, 0.8)
    floor = max(0.1, initial - 0.001)
    minimum = min((clearance(p) for p in curve))
    length = sum((math.dist(a[:2], b[:2]) for (a, b) in zip(curve, curve[1:])))
    if initial < 0.1 or minimum < floor:
        raise RuntimeError('Reverse clearance screening failed')
    if clearance(curve[-1]) < initial + 0.05:
        raise RuntimeError('Reverse does not create sufficient clearance')
    if any(((p[0] - cx) * nx + (p[1] - cy) * ny >= 0 for p in curve)):
        raise RuntimeError('Reverse crosses the gap plane')
    if not reverse_grid_audit(curve, grid_snapshot()):
        raise RuntimeError('Reverse occupancy screening failed')
    unchanged(anchor)
    check_frame()
    ...
    update_parameters([ParameterValue(type=1, bool_value=True), ParameterValue(type=1, bool_value=False), ParameterValue(type=3, double_value=0.6)])
    unchanged(anchor)
    check_frame()
    if STOP:
        raise StageFailure('INTERRUPTED', 'Interrupted')
    goal = FollowPath.Goal()
    goal.path = as_path(curve)
    goal.controller_id = 'FollowPath'
    goal.goal_checker_id = 'alignment_goal_checker'
    print(f'TRIKE WILL REVERSE along {length:.3f}m curve; audited minimum={minimum:.4f}m; max curvature={max_curvature:.3f}/m', flush=True)
    send_future = follower.send_goal_async(goal)
    active = wait(send_future, 15)
    if not active.accepted:
        raise RuntimeError('Reverse alignment rejected')
    result_future = active.get_result_async()
    started = last_print = last_grid = time.monotonic()
    observed_min = initial
    while not result_future.done():
        spin(0.05)
        if STOP or time.monotonic() - started > 60:
            raise StageFailure('INTERRUPTED', 'Interrupted or alignment timeout')
        check_frame()
        current = pose()
        clr = clearance(current)
        observed_min = min(observed_min, clr)
        nearest = min(range(len(curve)), key=lambda i: math.dist(current[:2], curve[i][:2]))
        xte = min((geo['pt_seg'](current[:2], a[:2], b[:2]) for (a, b) in zip(curve, curve[1:])))
        if clr < floor:
            raise RuntimeError(f'Reverse clearance guard: {clr:.4f}m')
        if xte > 0.05:
            raise RuntimeError(f'Reverse tracking guard: {xte:.4f}m')
        odom_pose()
        for key in ('NAV', 'CM', 'LIM'):
            entry = state.get(key)
            if entry and time.monotonic() - entry[2] < 0.3 and (entry[0] > 0.01):
                raise StageFailure('REVERSE_ENDPOINT', 'Unexpected forward command during reverse')
        if time.monotonic() - last_grid >= 0.5:
            last_grid = time.monotonic()
            remaining = [current] + curve[max(0, nearest - 2):]
            if not reverse_grid_audit(remaining, grid_snapshot(moving=True)):
                raise RuntimeError('Reverse occupancy changed')
        if time.monotonic() - last_print >= 0.5:
            last_print = time.monotonic()
            print(f't={last_print - started:5.1f} clr={clr:.4f} xte={xte:.4f} target={math.dist(current[:2], target[:2]):.3f}m heading={math.degrees(wrap(current[2] - ch)):+.2f}deg', flush=True)
    status = result_future.result().status
    finish_motion()
    if status != 4:
        raise RuntimeError(f'Alignment action ended with status {status}')
    refresh_stationary_pose()
    measure_scene()
    actual = anchor
    lateral = -(actual[0] - cx) * ny + (actual[1] - cy) * nx
    heading = math.degrees(wrap(actual[2] - ch))
    setback = -((actual[0] - cx) * nx + (actual[1] - cy) * ny)
    print(f'ACHIEVED: setback={setback:.4f}m lateral={lateral:+.4f}m heading={heading:+.3f}deg; minimum observed clearance={observed_min:.4f}m', flush=True)
    centred = (cx - setback * nx, cy - setback * ny, ch)
    reference = line_samples(centred, exit_pose)
    reference_min = min((clearance(p) for p in reference))
    grid_ok = reverse_grid_audit(reference, grid_snapshot())
    print(f'Centred reference passage: minimum={reference_min:.4f}m; occupancy clear={grid_ok}', flush=True)
    print('Reverse test complete. Forward passage NOT started. The measured alignment above determines the next passage path.', flush=True)
    report.data['code'] = 'SUCCESS'
except Exception as exc:
    report.fail(exc)
    if STOP:
        report.data['code'] = 'INTERRUPTED'
    print('TEST STOPPED:', exc, flush=True)
finally:
    try:
        finish_motion()
        ...
        final_pose = refresh_stationary_pose()
        report.data['metrics']['achieved_pose_map'] = list(final_pose)
        if 'target' in globals():
            report.data['metrics'].update(endpoint_distance=math.dist(final_pose[:2], target[:2]), heading_error_deg=math.degrees(wrap(final_pose[2] - target[2])))
    except Exception as exc:
        print('CLEANUP INCOMPLETE:', exc, 'Stop navigation if moving; verify controller parameters.', flush=True)
