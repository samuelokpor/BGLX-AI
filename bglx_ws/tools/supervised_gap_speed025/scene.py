...
def spawn_scene():
    ...
    measure_scene()
    reference = anchor
    measured_centres = [(sum((p[0] for p in box)) / len(box), sum((p[1] for p in box)) / len(box)) for box in boxes]
    print('Measured pillar centres in map:', measured_centres, flush=True)
    deadline = time.monotonic() + 25
    last_print = 0.0
    last_reason = 'No global costmap received'
    while time.monotonic() < deadline:
        if STOP:
            raise StageFailure('INTERRUPTED', 'Interrupted')
        spin(0.2)
        unchanged(reference)
        check_frame()
        entry = audit_grids.get('global')
        if entry is None:
            last_reason = 'No global raw costmap messages'
        else:
            (m, received) = entry
            now = n.get_clock().now().nanoseconds
            age = (now - Time.from_msg(m.header.stamp).nanoseconds) / 1000000000.0
            receive_age = time.monotonic() - received
            if m.header.frame_id != 'map':
                last_reason = f'Unexpected costmap frame: {m.header.frame_id}'
            elif not -0.05 <= age <= 4 or receive_age > 4:
                last_reason = f'Costmap timing: stamp_age={age:.3f}s receive_age={receive_age:.3f}s'
            else:
                md = m.metadata
                grid = np.asarray(m.data, dtype=np.uint8).reshape(md.size_y, md.size_x)
                (jj, ii) = np.nonzero(grid == 254)
                a = yaw(md.origin.orientation)
                u = (ii + 0.5) * md.resolution
                v = (jj + 0.5) * md.resolution
                x = md.origin.position.x + math.cos(a) * u - math.sin(a) * v
                y = md.origin.position.y + math.sin(a) * u + math.cos(a) * v
                counts = []
                for (bx, by) in measured_centres:
                    (dx, dy) = (x - bx, y - by)
                    counts.append(int(((abs(dx * nx + dy * ny) < 0.4) & (abs(-dx * ny + dy * nx) < 0.4)).sum()))
                last_reason = f'left/right lethal cells={counts}; stamp_age={age:.3f}s; receive_age={receive_age:.3f}s; resolution={md.resolution:.3f}m'
                if min(counts) >= 4:
                    print('OBSERVATION PASSED:', last_reason, flush=True)
                    return
        if time.monotonic() - last_print >= 2:
            last_print = time.monotonic()
            print('OBSERVATION WAIT:', last_reason, flush=True)
    raise RuntimeError('Pillar observation failed: ' + last_reason)
...
(GAP, DISTANCE, OFFSET) = (1.4, 6.0, 0.5)
...
try:
    import os
    (GAP, OFFSET) = (1.4, 0.5)
    (DISTANCE, ANGLE) = (6.0, math.radians(15)) if os.environ['BGLX_SCENE'] == 'mirror15' else (5.0, math.radians(25))
    if os.environ['BGLX_SCENE'] == 'approach25_offset1':
        (GAP, DISTANCE, OFFSET) = (1.4, 5.0, 1.0)
        ANGLE = math.radians(25)
    (GAP, DISTANCE, OFFSET) = (1.6, 5.0, 1.0)
    ANGLE = math.radians(25)
    if os.environ['BGLX_SCENE'] == 'speed025_angle35':
        (GAP, DISTANCE, OFFSET) = (1.6, 5.0, 1.0)
        ANGLE = math.radians(35)
    if os.environ['BGLX_SCENE'] == 'speed025_mirror35':
        (GAP, DISTANCE, OFFSET) = (1.6, 5.0, -1.0)
        ANGLE = math.radians(-35)
    print(f'V2 SCENE: gap={GAP} distance={DISTANCE} offset={OFFSET} angle={math.degrees(ANGLE)}', flush=True)
    print('25 DEGREE NEAR SETUP: 1.4m gap, 5m ahead, +0.5m offset, +25deg', flush=True)
    deadline = time.monotonic() + 25
    while True:
        try:
            refresh_stationary_pose()
    ...
    refresh_stationary_pose()
    measure_scene()
    print('APPROACH25_NEAR SCENE READY — no motion requested', flush=True)
    scene_status = 0
    report.data['code'] = 'SUCCESS'
except Exception as exc:
    report.fail(exc)
    if STOP:
        report.data['code'] = 'INTERRUPTED'
    print('SCENE SETUP FAILED:', exc, flush=True)
finally:
    try:
        if not wait_stopped():
            raise RuntimeError('Scene stop unconfirmed')
        report.data.update(stopped=True, cleanup_ok=True)
    except Exception as cleanup_error:
        report.fail(cleanup_error)
    n.destroy_node()
    rclpy.shutdown()
report.finish()
