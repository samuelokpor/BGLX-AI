...
run = FilePath(os.environ.get('BGLX_COURSE_RUN', str(FilePath.home() / 'bglx_navtest/logs' / ('four_resume_' + uuid.uuid4().hex))))
run.mkdir(parents=True)
os.environ.update(BGLX_RESULT=str(run / 'result.json'), BGLX_RUN_ID=run.name, BGLX_STAGE='forward')
...
def course_audit(points):
    ...
    if min(minima) < 0.17 or reverse > 0.01 or len(crossings[0]) != 0 or (len(crossings[1]) != 1):
